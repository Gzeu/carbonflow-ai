...
/**
 * Returns the most recent carbon event for a given repo + event_type.
 * Used for PR trend comparison (Wave 3).
 *
 * @param {string} repo       - e.g. "Gzeu/carbonflow-ai"
 * @param {string} eventType  - "push" | "pull_request" | "workflow_run"
 * @returns {Promise<{energy_kwh: number, carbon_kg: number, tier: string, numeric_score: number|null, created_at: string}|null>}
 */
export async function getLastEvent(repo, eventType) {
  ...
  const { data, error } = await db
    .from('carbon_events')
    .select('energy_kwh, carbon_kg, tier, meta, created_at')
    .eq('repo', repo)
  ...
  if (error) {
    logger.warn('getLastEvent query failed', { error: error.message, repo, eventType });
    return null;
  }
